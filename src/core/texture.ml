...
let image_from_surface ctx surface x y w h dw dh =
  (**
      [image_from_surface ctx surface x y w h dw dh]*)
  let dst = Gfx.create_surface ctx dw dh in
  Gfx.blit_full ctx dst surface x y w h 0 0 dw dh;
  Image dst


  let anim_from_surface ctx surface n w h dw dh fd = 
    let frames = 
      Array.init n (fun i -> 
        let dst = Gfx.create_surface ctx dw dh in 
        Gfx.blit_full ctx dst surface (i*w) 0 w h 0 0 dw dh;
        dst)
      in Animation { frames; frame_duration = fd; current_time = fd ; current_frame = 0}